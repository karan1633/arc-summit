...
const useLoginHook = () => {
  const { ARC_APP_CONFIG, SUMMIT_APP_CONFIG } = CONSTANTS;
  ...
  const fetchToken = async (values: TypeLoginForm) => {
    ...
    const tokenData = await getTokenFromLoginAPI(ARC_APP_CONFIG,SUMMIT_APP_CONFIG ,userParams);
    if (tokenData?.hasOwnProperty('access_token')) {
      const pageHistory = { reference_type: 'Login', reference_id: 'login' };
      localStorage.setItem('isLoggedIn', 'true');
      dispatch(storeToken(tokenData));
      localStorage.setItem('user', values.usr);
      localStorage.setItem('party_name', tokenData?.full_name);
      localStorage.setItem('party_id', tokenData?.party_id);
